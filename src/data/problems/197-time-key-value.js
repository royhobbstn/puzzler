...
TimeMap.prototype.set = function (key, value, timestamp) {
  ...
  // push value into created array.  Timestamps are sequential, so these
  // are automatially inserted in order from low to high
...
TimeMap.prototype.get = function (key, timestamp) {
  ...
  // conduct a binary search on the array at hash[key]
  let start = 0;
  let end = arr.length - 1;
  let mid;

  // typical binary search, though we keep track of 'mid' outside of while loop
  while (start <= end) {
  ...
  // we're here if there was no exact match
  // check the value at the last middle index, if it is less than timestamp
  // then return it
  // else check value at middle - 1 index, if it exists, return it
  // if not (probably due to out of index error) return blank string
