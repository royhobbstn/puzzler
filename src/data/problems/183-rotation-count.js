...
const solution = [
  ...
  { stage: 0, text: '' },
  { stage: -4, text: '    // check to see if mid or mid+1 is the minimum element' },
  {
    stage: -4,
    text:
      '    // the minimum element is the only element which is smaller than its previous element.',
  },
  { stage: 0, text: '' },
  { stage: -4, text: '    // we check mid<end because if mid===end, there is no mid+1' },
  { stage: 4, text: '    if (mid < end && arr[mid] > arr[mid + 1]) {' },
  { stage: 5, text: '      return mid + 1;' },
  { stage: 4, text: '    }' },
  { stage: 0, text: '' },
  { stage: -6, text: '    // we check mid>start because if mid===start, there is no mid-1' },
  { stage: 6, text: '    if (mid > start && arr[mid - 1] > arr[mid]) {' },
  { stage: 7, text: '      return mid;' },
  { stage: 6, text: '    }' },
  { stage: 0, text: '' },
  { stage: -8, text: '    // typical binary search from this point on' },
